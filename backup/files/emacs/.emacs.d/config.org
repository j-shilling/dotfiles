...
*** Helpful
#+begin_src emacs-lisp
  (when (init-package-installed-p 'helpful)
...
#+begin_src emacs-lisp
  ...
  (add-hook 'init-first-input-hook
            (cond
             ((init-package-installed-p 'vertico) #'vertico-mode)
             ((fboundp 'fido-vertical-mode) #'fido-vertical-mode)
             ((fboundp 'fido-mode) #'fido-mode)))
  ...
  (when (init-package-installed-p 'orderless)
    (setq completion-styles '(orderless basic)))

  (when (init-package-installed-p 'consult)
  ...
  (when (init-package-installed-p 'marginalia)
    (add-hook 'init-first-input-hook
              #'marginalia-mode))
  ...
  (when (init-package-installed-p 'yasnippet)
    ...
    (after! yasnippet
      (setq yas-key-syntaxes (list #'yas-try-key-from-whitespace
                                   "w_.()" "w_." "w_"))
...
** EShell
   :PROPERTIES:
   :header-args: :tangle modules/init-eshell.el
   :END:
   ...
   #+begin_src emacs-lisp
     ...
     (after! eshell
       ...
       (declare-function init-state-path (& parts))
       ...
       ;; Use tramp for things like `su' or `sudo'
       (require 'esh-module)
       (defvar eshell-modules-list)
       (add-to-list 'eshell-modules-list 'eshell-tramp)

       (declare-function init-eshell-require-tramp ())
       (defun init-eshell-require-tramp ()
         (require 'tramp)
         (remove-hook 'eshell-mode-hook #'init-eshell-require-tramp))
       (add-hook 'eshell-mode-hook #'init-eshell-require-tramp)
       ...
       (when (file-exists-p "~/.config/guix")
         (let ((guile-load-path (getenv "GUILE_LOAD_PATH")))
           (setenv "GUILE_LOAD_PATH"
                   (concat "~/.config/guix:" guile-load-path))))

       (define-key eshell-mode-map (kbd "<tab>") 'completion-at-point))

     (when (init-package-installed-p 'esh-autosuggest)
       (declare-function esh-autosuggest-mode ())
       (autoload 'esh-autosuggest-mode "esh-autosuggest"
         "Enable fish-like-autosuggestions in eshell"t)

       (add-hook 'eshell-mode-hook #'esh-autosuggest-mode))

     (when (init-package-installed-p 'eshell-syntax-highlighting)
       (after! eshell
         (eshell-syntax-highlighting-global-mode +1)))

     (when (init-package-installed-p 'xterm-color)
       (after! eshell
         (require 'esh-mode)
         (require 'xterm-color)
     ...
     (when (init-package-installed-p 'envrc)
...
** Dired and Proced
#+begin_src emacs-lisp
  ;;; init-dired.el -- Configure Dired -*- lexical-bidning: t -*-
  ...
  (when (init-package-installed-p 'all-the-icons-dired)
...
** Spelling
:PROPERTIES:
:header-args: :tangle modules/init-spelling.el
:END:

#+begin_src emacs-lisp
  ;;; init-spelling.el -- Configure Flyspell -*- lexcial-binding: t -*-
...
:END:
   ...
   #+begin_src emacs-lisp
     ...
     (when (init-package-installed-p 'yasnippet)
       ...
       (defun init-lisp-non-whitespace-char-before (&optional pos)
         "Return the char before POS, ignoring whitespace.

     If POS is nil, then uses `point'."
         (save-excursion
           (when pos
             (goto-char pos))
       ...
       (defun init-lisp-non-whitespace-char-after (&optional pos)
         "Return the char after POS, ignoring whitespace.
     ...
     If POS is nil, then uses `point'."
       ...
       (defun init-lisp-point-in-sexp-p (&optional start end)
         "Return non-nil when parens are around START and END."
         (and (eq ?\( (init-lisp-non-whitespace-char-before start))
              (eq ?\) (init-lisp-non-whitespace-char-after end))))

       (defun init-lisp-string-is-sexp-p (s)
         (string-match "\s*(.*)\s*" s nil t))

       (defun init-lisp-check-parens ()
         (save-excursion
           (when (and (init-lisp-point-in-sexp-p (if (boundp 'start)
                                                     start
                                                   (error "'start is not bound"))
                                                 (if (boundp 'end)
                                                     end
                                                   (error "'end is not bound")))
                      (init-lisp-string-is-sexp-p
                       (yas--template-content snippet)))
             (setf start (scan-lists (point) -1 +1)
                   end (scan-lists (point) +1 +1)))))

       (after! yasnippet
         (add-hook 'yas-before-expand-snippet-hook #'init-lisp-check-parens)))
